...
Above the charts, the panel reports the top focal regions, top [mapzebrain](https://mapzebrain.org) atlas regions, top transcriptomic clusters, and how the selection partitions across the 3 source specimens as counts. A selection dominated by a single specimen warrants a check that the finding holds across all three.
