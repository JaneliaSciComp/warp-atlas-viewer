...
| Group | Examples |
|---|---|
| **Filter cards** | Color scheme, region palette, pinned gene indices, gene OR/AND, subtype index, selected stimuli, stim mode, swim mode, region, atlas region ([mapzebrain](https://mapzebrain.org)), specimen, show-unassigned-region toggle. |
| **Settings** | 3D point density (auto, scale by filter, manual point size + ghost visibility), 3D camera controls (object-centric rotation, momentum), t-SNE point density (size + ghost visibility), rendering options (3D ambient occlusion, AO strength/radius, opaque active cells, active brightness), fade-weak-correlation toggle, stim cutoffs, swim cutoffs, activity anchors, gene-spot ceiling, multi-gene mode, gene predicate, debug-overlay toggle, activity playback speed. |
