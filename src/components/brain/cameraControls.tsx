...
/** A centred, unpanned frustum. A module constant so it is referentially
 *  stable in effect dependency arrays. */
export const NO_PAN: ScreenPanState = { x: 0, y: 0 };

/** The projection offset for a given pan. Negated because `setViewOffset`'s
 *  x/y move the *viewing window*, so a positive offset moves the rendered
 *  volume up/left — the opposite of ScreenPanState's convention, where positive
 *  moves the volume right/down. Exported for its own unit test: this sign is
 *  the easiest thing in the file to get backwards, and getting it backwards
 *  shifts the brain the wrong way by exactly the amount asked for, which is
 *  hard to notice. */
export function viewOffsetFor(pan: ScreenPanState): { x: number; y: number } {
  ...
  return { x: flip(pan.x), y: flip(pan.y) };
}

/** Applies `pan` as a projection offset. Returns false when the camera or
 *  canvas cannot take an offset yet. */
...
export function ScreenSpacePan({
  ...
  const applyViewOffset = useCallback(() => {
    if (setPanViewOffset(camera, size, panRef.current)) invalidate();
  }, [camera, invalidate, panRef, size]);

  useEffect(() => {
    if (!enabled) {
      // Drop the user's pan so the native trackball pan sees a centered
      // frustum.
      if (setPanViewOffset(camera, size, NO_PAN)) invalidate();
      return;
    }
    applyViewOffset();
  }, [applyViewOffset, camera, enabled, invalidate, size]);
...
export function CameraSync({
  ...
  useEffect(() => {
    applyViewRef.current = (position, up) => {
      ...
      panRef.current.x = 0;
      panRef.current.y = 0;
      setPanViewOffset(camera, size, NO_PAN);
      invalidate();
    };
    return () => {
      applyViewRef.current = null;
    };
  }, [camera, controls, invalidate, panRef, applyViewRef, size, volumeCenter]);
